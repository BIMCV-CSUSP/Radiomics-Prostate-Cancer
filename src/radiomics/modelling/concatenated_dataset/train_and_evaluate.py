...
def main():
    parser = argparse.ArgumentParser(description="Evaluación de modelos con validación cruzada repetida")
    parser.add_argument("--csv", type=str, default="features_all_gland.csv",
                        choices=["features_all_gland.csv", "features_all_full.csv"],
                        help="Ruta al CSV con las características.")
    
    # n_splits => cuántos folds por repetición
    parser.add_argument("--n_splits", type=int, default=10, # 10
                        help="Número de particiones para StratifiedGroupKFold (por repetición).")
    # n_repeats => cuántas veces repetamos la división
    parser.add_argument("--n_repeats", type=int, default=5, # 5
                        help="Número de repeticiones de la validación cruzada.")
    
    parser.add_argument("--feature_strategy", type=str, default="all",
                        choices=["all", "most_discriminant"],
                        help="Estrategia de features: 'all' o 'most_discriminant'.")
    
    # Opcionales
    parser.add_argument("--calculate_differences", action="store_true", default=True,
                        help="Si se habilita, ejecuta model_differences.py (por defecto: True)")
    parser.add_argument("--fine_tune_best_model", action="store_true", default=False,
                        help="Si se habilita, realiza un fine-tuning adicional del mejor modelo (por defecto: False)")

    args = parser.parse_args()
    
    # --- Preparamos la ruta al CSV ---
    pre_path = "../../../../../data/radiomic_data/"
    if args.csv:
        data_path = os.path.join(pre_path, "concatenated_data", args.csv)
        csv_filename = args.csv
    else:
        data_path = os.path.join(pre_path, "concatenated_data", "features_all_gland.csv")
        csv_filename = "features_all_gland.csv"
    ...
    strategy_dir = os.path.join(base_results_dir, args.feature_strategy)
    ...
    csv_dir = os.path.join(strategy_dir, csv_name_no_ext)
    if not os.path.exists(csv_dir):
        os.mkdir(csv_dir)
    
    existing_experiments = [
        d for d in os.listdir(csv_dir)
        if os.path.isdir(os.path.join(csv_dir, d)) and d.startswith("experiment_")
    ]
    experiment_number = len(existing_experiments) + 1
    experiment_dir = os.path.join(csv_dir, f"experiment_{experiment_number}")
    ...
    if args.fine_tune_best_model:
        if len(curves_info_optimal) > 0:
            ...
            fine_tune_cmd = [
                "python3",
                "retrain_best_model_and_evaluate.py",
                "--csv", args.csv,                       
