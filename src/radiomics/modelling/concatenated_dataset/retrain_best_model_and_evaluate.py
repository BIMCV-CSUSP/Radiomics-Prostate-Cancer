...
def main():
    ...
    #     top_features = X_transformed.columns[top_idx]
    ...
    try:
        ...
        # 2) Iteramos en múltiples instancias (cuidado con el costo computacional)
        for i in range(100):
            ...
            for (feat_str, peso) in lime_list:
                ...
                valor_feature = X_test_lime[i, col_idx]
        ...
        df_lime_top15 = df_lime[df_lime['feature'].isin(top_features)]
        
        # 3) Crear bins para 'valor_feature'
        num_bins = 10
        df_lime_top15['valor_binned'] = pd.cut(df_lime_top15['valor_feature'], bins=num_bins)
        
        # 4) Graficar con stripplot y hue="valor_binned"
        ...
        sns.stripplot(
            ...
            y='feature',
            hue='valor_binned',
            palette='coolwarm',  # Ej. "coolwarm", "RdBu_r", "seismic"
            orient='h',
            size=5
        )
        plt.axvline(0, color='black', linestyle='--')
        plt.title("Distribución de pesos LIME (bins de valor) - Top 15 features")
        plt.tight_layout()
        
        # 5) Guardar la figura
        fig_path = os.path.join(lime_dir, "lime_binned_top15.png")
        plt.savefig(fig_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"Gráfico binned (Top 15 features) guardado en: {fig_path}")

        # 5) Generamos explicaciones locales para algunas instancias.
        np.random.seed(42)
        sample_indices = np.random.choice(len(X_test_lime), size=5, replace=False)
        ...
        for i, idx in enumerate(sample_indices, start=1):
            ...
            explanation_txt_path = os.path.join(lime_dir, f"lime_explanation_{i}.txt")
            with open(explanation_txt_path, "w", encoding="utf-8") as f:
                f.write("=== LIME Explanation (instancia índice: {}) ===\n".format(idx))
                f.write(f"Predicción del modelo: {model_clf.predict([X_test_lime[idx]])}\n")
                f.write(f"Probabilidades: {model_clf.predict_proba([X_test_lime[idx]])}\n\n")
            ...
            plt.title(f"LIME Explanation - Instancia {idx}")
            fig_path = os.path.join(lime_dir, f"lime_explanation_{i}.png")
            plt.savefig(fig_path, dpi=300, bbox_inches='tight')
            plt.close()
    
            print(f"  -> LIME para instancia {idx} guardado en:\n     {explanation_txt_path}\n     {fig_path}")
